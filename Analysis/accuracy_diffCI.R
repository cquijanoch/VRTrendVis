propDiffCI <- function(result.df,task_num) {
    ...
    #clm <- smean.cl.boot(df[df$VisModeID=='Mix',]$accuracy, B=10000, reps=TRUE)
    cls <- smean.cl.boot(df[df$VisModeID=='SMultiples',]$accuracy, B=10000, reps=TRUE)
    clo <- smean.cl.boot(df[df$VisModeID=='Overlaid',]$accuracy, B=10000, reps=TRUE)
    cla <- smean.cl.boot(df[df$VisModeID=='Animation',]$accuracy, B=10000, reps=TRUE)
    ...
    #bs.m <- quantile(s-m,c((1-0.994),0.994))

    #o.m <- quantile(o-m,c(.05,.95))
    #bo.m <- quantile(o-m,c((1-0.994),0.994))

    o.s <- quantile(o-s,c(.05,.95))
    #bo.s <- quantile(o-s,c((1-0.994),0.994))
    bo.s <- quantile(o-s,c(.01,0.99))

    #a.m <- quantile(a-m,c(.05,.95))
    #ba.m <- quantile(a-m,c((1-0.994),0.994))

    a.s <- quantile(a-s,c(.05,.95))
    #ba.s <- quantile(a-s,c((1-0.994),0.994))
    ba.s <- quantile(a-s,c(.01,0.99))

    a.o <- quantile(a-o,c(.05,.95))
    #ba.o <- quantile(a-o,c((1-0.994),0.994))
    ba.o <- quantile(a-o,c(.01,0.99))
